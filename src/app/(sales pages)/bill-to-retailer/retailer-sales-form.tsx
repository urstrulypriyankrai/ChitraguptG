...
export default function RetailerSalesForm({
  ...
  useEffect(() => {
    ...
    const random = Math.floor(Math.random() * 1000)
      .toString()
      .padStart(3, "0");
    setBillNumber(`R${year}${month}${day}${random}`);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedRetailer) {
      toast({ title: "Please select a retailer", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/sales/retailer", {
        method: "POST",
        headers: {
    ...
    try {
      ...
      if (navigator.share && pdfBlob) {
        ...
        await navigator.share({
          ...
          text: `Bill for ${retailerDetails?.name || "Retailer"}`,
          files: [file],
        });
      } else if (pdfBlob) {
        // Fallback for browsers that don't support navigator.share with files
        const pdfUrl = URL.createObjectURL(pdfBlob);
        const phoneNumber = retailerDetails?.mobile;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <Select value={selectedRetailer} onValueChange={setSelectedRetailer}>
            <SelectTrigger id="retailer" className="mt-1">
              <SelectValue placeholder="Select a retailer" />
            </SelectTrigger>
            <SelectContent>
              {retailers.map((retailer) => (
                <SelectItem key={retailer.id} value={retailer.id}>
                  {retailer.name} - {retailer.gstNumber || "No GST"}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          <CardContent className="pt-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <p>{retailerDetails.name}</p>
              </div>
              <div>
                <p className="text-sm font-medium">GST Number</p>
                <p>{retailerDetails.gstNumber || "N/A"}</p>
              </div>
              <div>
                <p className="text-sm font-medium">Address</p>
                <p>
                  {[
                    retailerDetails.address?.street,
                    retailerDetails.address?.district,
                    retailerDetails.address?.state,
                    retailerDetails.address?.zip,
                  ]
                    .filter(Boolean)
                    .join(", ")}
                </p>
        ...
        <div className="flex flex-col justify-end space-y-4">
          <div className="flex flex-wrap gap-2 justify-end">
            <Button
              ...
              disabled={selectedItems.length === 0 || !selectedRetailer}
            >
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button
              type="submit"
              disabled={
                isSubmitting || selectedItems.length === 0 || !selectedRetailer
              }
            >
